...
async function assertAdmin(uid) {
  ...
  const claimAdmin = authUser.customClaims?.admin === true || authUser.customClaims?.role === "admin";
...
function deriveWalletFromTransactions(docs) {
  ...
  for (const doc of docs) {
    ...
    if ((ty === "profit" || ty === "profit_entry") && (st === "approved" || st === "completed")) totalProfit += amt;
    if (ty === "adjustment" && (st === "approved" || st === "completed")) totalAdjustments += amt;
  }

  const availableBalance = totalDeposited + totalProfit + totalAdjustments - totalWithdrawn - reservedAmount;
...
exports.createDepositRequest = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  const uid = request.auth.uid;
  const { amount, paymentMethod, proofUrl } = request.data || {};
  const amt = Number(amount);
  if (!amt || amt <= 0) throw new HttpsError("invalid-argument", "Invalid amount.");
  ...
  await safeAppendAudit(uid, "investor", "createDepositRequest", "deposit_request", reqRef.id, null, {
    amount: amt,
    transactionId: tid,
  });
...
exports.createWithdrawalRequest = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  const uid = request.auth.uid;
  ...
  await db().runTransaction(async (tx) => {
    const txsSnap = await tx.get(db().collection("transactions").where("userId", "==", uid));
  ...
  await recalculateWallet(uid);
  await safeAppendAudit(uid, "investor", "createWithdrawalRequest", "withdrawal_request", reqRef.id, null, {
    amount: amt,
    transactionId: tid,
  });
...
exports.approveDeposit = onCall({ region: "us-central1" }, async (request) => {
  ...
  const reqSnap = await db().collection("deposit_requests").doc(requestId).get();
  ...
  const batch = db().batch();
  batch.update(reqSnap.ref, { status: "approved", updatedAt: now, adminNote: note || null });
  batch.update(tRef, {
    status: "approved",
    updatedAt: now,
    notes: note || null,
...
exports.rejectDeposit = onCall({ region: "us-central1" }, async (request) => {
  ...
  const batch = db().batch();
  batch.update(reqSnap.ref, { status: "rejected", updatedAt: now, rejectReason: reason || "" });
  batch.update(tRef, {
    status: "rejected",
    updatedAt: now,
  ...
  await batch.commit();
  await recalculateWallet(req.userId);
  await safeAppendAudit(request.auth.uid, "admin", "rejectDeposit", "deposit_request", requestId, null, {
    reason,
  });
...
exports.approveWithdrawal = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  await assertAdmin(request.auth.uid);
  const { requestId, note } = request.data || {};
  if (!requestId) throw new HttpsError("invalid-argument", "requestId required.");

  const reqSnap = await db().collection("withdrawal_requests").doc(requestId).get();
  if (!reqSnap.exists) throw new HttpsError("not-found", "Request not found.");
  const req = reqSnap.data();
  if (req.status !== "pending") {
    throw new HttpsError("failed-precondition", "Request not pending.");
  ...
  await batch.commit();
  await recalculateWallet(req.userId);
  await safeAppendAudit(request.auth.uid, "admin", "approveWithdrawal", "withdrawal_request", requestId, null, {
    status: "approved",
  });
...
exports.completeWithdrawal = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  await assertAdmin(request.auth.uid);
  ...
  if (req.status !== "approved") {
    throw new HttpsError("failed-precondition", "Request must be approved first.");
  }
  const tid = req.transactionId;
  const tRef = db().collection("transactions").doc(tid);
  const now = admin.firestore.FieldValue.serverTimestamp();
  ...
  const batch = db().batch();
  batch.update(reqSnap.ref, { status: "completed", updatedAt: now, settlementRef: settlementRef || null });
  batch.update(tRef, {
    status: "completed",
    updatedAt: now,
  ...
  await batch.commit();
  await recalculateWallet(req.userId);
  await safeAppendAudit(request.auth.uid, "admin", "completeWithdrawal", "withdrawal_request", requestId, null, {
    status: "completed",
  });
...
exports.rejectWithdrawal = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  await assertAdmin(request.auth.uid);
  const { requestId, reason } = request.data || {};
  if (!requestId) throw new HttpsError("invalid-argument", "requestId required.");
  ...
  if (req.status !== "pending" && req.status !== "approved") {
    throw new HttpsError("failed-precondition", "Request cannot be cancelled.");
  }
  const tid = req.transactionId;
  const tRef = db().collection("transactions").doc(tid);
  const now = admin.firestore.FieldValue.serverTimestamp();
  ...
  const batch = db().batch();
  batch.update(reqSnap.ref, { status: "cancelled", updatedAt: now, adminNote: reason || null });
  batch.update(tRef, {
    status: "cancelled",
    updatedAt: now,
  ...
  await batch.commit();
  await recalculateWallet(req.userId);
  await safeAppendAudit(request.auth.uid, "admin", "rejectWithdrawal", "withdrawal_request", requestId, null, {
    reason,
  });
...
exports.addProfitEntry = onCall({ region: "us-central1" }, async (request) => {
  ...
  await recalculateWallet(userId);
  await safeAppendAudit(request.auth.uid, "admin", "addProfitEntry", "transaction", tid, null, { userId, amount: amt });

  return { transactionId: tid };
});

/** Admin: adjustment entry (signed amount) */
exports.addAdjustmentEntry = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  await assertAdmin(request.auth.uid);
  const { userId, amount, note } = request.data || {};
  if (!userId) throw new HttpsError("invalid-argument", "userId required.");
  const amt = Number(amount);
  if (Number.isNaN(amt) || amt === 0) throw new HttpsError("invalid-argument", "Invalid adjustment amount.");
  if (!note || String(note).trim().length < 3) {
    throw new HttpsError("invalid-argument", "Note required for adjustments.");
  }
  ...
  await recalculateWallet(userId);
  await safeAppendAudit(request.auth.uid, "admin", "addAdjustmentEntry", "transaction", tid, null, {
    userId,
    amount: amt,
  });
...
exports.recalculateWalletForUser = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  await assertAdmin(request.auth.uid);
...
exports.adminApproveTransaction = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  await assertAdmin(request.auth.uid);
  ...
  if (status !== "pending") {
    throw new HttpsError("failed-precondition", `Transaction is already ${status}.`);
  }
  ...
  await batch.commit();
  await recalculateWallet(userId);
  await safeAppendAudit(adminUid, "admin", "adminApproveTransaction", "transaction", txnId, { status: "pending" }, { status: "approved", type, amount });

  return { ok: true };
});

/** Admin: reject a transaction */
exports.adminRejectTransaction = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  await assertAdmin(request.auth.uid);
  ...
  await recalculateWallet(userId);
  await safeAppendAudit(adminUid, "admin", "adminRejectTransaction", "transaction", txnId, { status: "pending" }, { status: "rejected", rejectionNote });

  return { ok: true };
});

/** Admin: apply percentage return to ALL portfolios */
exports.applyMonthlyReturns = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  await assertAdmin(request.auth.uid);

  const returnPct = Number(request.data?.returnPct);
  if (!returnPct || returnPct <= 0 || returnPct > 100) {
    throw new HttpsError("invalid-argument", "returnPct must be between 0 and 100.");
  }
  ...
  for (const portfolioDoc of portfoliosSnap.docs) {
    ...
    try {
      ...
      const histRef = db().collection("portfolios").doc(uid).collection("returnHistory").doc();
      batch.set(histRef, {
        returnPct,
        profitAmount: profit,
        previousValue,
      ...
      logger.error("applyMonthlyReturns user failed", { uid, error: String(e) });
    }
  }

  await safeAppendAudit(adminUid, "admin", "applyMonthlyReturns", "portfolios", "all", null, {
    returnPct,
    successCount,
    failCount,
    totalProfit,
 ...
 */
exports.repairUserBalances = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  await assertAdmin(request.auth.uid);
  ...
  for (const userDoc of usersSnap.docs) {
    ...
    try {
      ...
      const oldWallet = (await db().collection("wallets").doc(uid).get()).data() || {};
      ...
      // Read new values
      const newWallet = (await db().collection("wallets").doc(uid).get()).data() || {};
      ...
      if (balanceChanged || withdrawnChanged) {
        ...
        logger.info("repairUserBalances: fixed", {
          ...
          withdrawnBefore: oldWithdrawn, withdrawnAfter: newWithdrawn,
        });
      } else {
      ...
      errorCount++;
      logger.error("repairUserBalances: error for user", { uid, error: String(err) });
...
exports.reconcileWalletsDaily = onSchedule("0 3 * * *", async () => {
  ...
  while (true) {
    ...
    for (const doc of usersSnap.docs) {
      ...
      if (wBefore && wAfter && Math.abs((wBefore.availableBalance || 0) - (wAfter.availableBalance || 0)) > 0.01) {
        logger.info("Wallet recalculated", { uid, before: wBefore, after: wAfter });
      }
    }
...
exports.getStorageImageData = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth?.uid) throw new HttpsError("unauthenticated", "Sign in required.");
  await assertAdmin(request.auth.uid);

  const rawUrl = String(request.data?.rawUrl || "").trim();
  if (!rawUrl) throw new HttpsError("invalid-argument", "rawUrl is required.");
  ...
  if (!objectPath) {
    throw new HttpsError("invalid-argument", "Could not parse storage object path.");
  }
